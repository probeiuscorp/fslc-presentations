...
In both these problems we want to ...
without "infecting" the layers between.

We don't want to end up with either Java's unchecked exceptions, or Java's checked exceptions.

So far we've looked at effects on data.
Now we focus more on effects on computation.

> Where's the line between "data" and "computation"?
...
**Koka** and **Eff** are two research languages made to study algebraic effects.
